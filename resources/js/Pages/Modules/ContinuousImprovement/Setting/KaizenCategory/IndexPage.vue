...
const headers = [
    ...
    {id: 'suggestions_count', label: tm('term.suggestionCount')},
...
const handleSubmit = async () => {
    ...
    if (formType.value === 'create') {
        form.post(route('kaizen-category.store'), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
        ...
        form.put(route('kaizen-category.update', {id: form.id}), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
                showModal.value = false;
...
const handleDelete = (id) => {
    router.delete(route("kaizen-category.destroy", id), {
        preserveState: true,
    });
}
</script>
...
<template>
    <setting-layout :title="tm('title.indexPage.title')" :sub-title="tm('title.indexPage.subTitle')" :links="links">
        <template #actionArea>
            <help-button title="Kaizen Kategorileri — Nasıl Çalışır?" subtitle="Kaizen önerilerinin sınıflandırması">
                <p>Her kaizen önerisi kaydedilirken buradaki listeden bir <strong>kategori</strong> (Verimlilik, Kalite, İş Güvenliği vb.) seçilir.</p>
