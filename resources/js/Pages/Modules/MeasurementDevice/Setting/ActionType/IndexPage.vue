...
const handleSubmit = async () => {
    ...
    if (formType.value === 'create') {
        form.post(route('measurement-device-action-type.store'), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
        ...
        form.put(route('measurement-device-action-type.update', {id: form.id}), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
                showModal.value = false;
...
const handleDelete = (id) => {
    router.delete(route("measurement-device-action-type.destroy", id), {
        preserveState: true,
    });
}
</script>
...
<template>
    <setting-layout :title="tm('title.indexPage.title')" :sub-title="tm('title.indexPage.subTitle')" :links="links">
        <template #actionArea>
            <help-button title="Cihaz Aksiyon Türleri — Nasıl Çalışır?" subtitle="Cihaz aksiyon geçmişinde kullanılan tür listesi">
                <p>Bir cihaza aksiyon (bakıma gönderme, arıza, tamir vb.) kaydederken buradaki listeden bir <strong>tür</strong> seçilir.</p>
