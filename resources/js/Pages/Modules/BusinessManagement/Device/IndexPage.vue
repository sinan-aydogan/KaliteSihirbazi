...
const handleSubmit = async () => {
    ...
    if (formType.value === 'create') {
        ...
        form.put(route('device.update', {device: form.id}), {
            onSuccess: () => {
                resetForm();
                showModal.value = false;
            },
...
const getRowInfo = (id) => {
    axios.get(route("device.edit", {device: id})).then(response => {
        form.id = response.data.id;
        form.code = response.data.code;
        form.name = response.data.name;
        form.device_type_id = response.data.device_type_id;
...
const handleDelete = (id) => {
    router.delete(route("device.destroy", id), {preserveState: true});
}
</script>
...
<template>
    <app-layout :title="tm('title.indexPage.title')" :sub-title="tm('title.indexPage.subTitle')">
        <template #actionArea>
            <simple-button type="route" :link="route('device.deleted')" color="red">
                <font-awesome-icon icon="trash-can" class="mr-2"/>
                <span v-text="$t('term.deletedItems')"/>
            </simple-button>

            <simple-button type="route" :link="route('device-type.index')" color="blue">
        ...
        <Table
            ...
            @view="router.visit(route('device.show', $event.id))"
        ...
        <Modal v-model="showModal" :header="tm('title.indexPage.title')" closeable close-button @closed="handleModalClosed">
            <Form full-size>
                <FormSection bg-less>
                    ...
                    <input-group class="col-span-6" labelFor="device_type_id" :label="tm('term.type')">
                        <select-input v-model="form.device_type_id" :options="deviceTypes" optionLabel="name"/>
                    </input-group>
