...
const handleSubmit = async () => {
    ...
    if (formType.value === 'create') {
        form.post(route('device-type.store'), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
        ...
        form.put(route('device-type.update', {device_type: form.id}), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
                showModal.value = false;
...
const getRowInfo = (id) => {
    axios.get(route("device-type.edit", {device_type: id})).then(response => {
...
const handleDelete = (id) => {
    router.delete(route("device-type.destroy", id), {preserveState: true});
}
</script>
