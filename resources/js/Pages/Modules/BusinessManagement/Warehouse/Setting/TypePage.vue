...
const handleSubmit = async () => {
  ...
  if (formType.value === 'create') {
    form.post(route('warehouse-type.store'), {
      onSuccess: () => {
        form.reset();
        v$.value.$reset();
    ...
    form.put(route('warehouse-type.update', {id: form.id}), {
      onSuccess: () => {
        form.reset();
        v$.value.$reset();
        showModal.value = false;
...
/*Update*/
const getRowInfo = (id) => {
  axios.get(route("warehouse-type.edit", {id: id})).then(response => {
...
<template>
  <SettingLayout
      ...
      :sub-title="tm('title.typeCreatePage.subTitle')"
      :links="links"
  >
    <template #actionArea>
