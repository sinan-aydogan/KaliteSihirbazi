...
<template>
    <app-layout :title="tm('title.deletedPage.title')" :sub-title="tm('title.deletedPage.subTitle')">
        <template #actionArea>
            <simple-button type="route" :link="route('machine.index')" color="blue">
        ...
        <Table
            ...
            @delete="router.delete(route('machine.permanent-delete', $event.id))"
            @restore="router.visit(route('machine.restore', $event.id))"
            restore-action
            delete-action
        >
