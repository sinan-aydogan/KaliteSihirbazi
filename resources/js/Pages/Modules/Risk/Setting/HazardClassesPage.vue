...
const handleSubmit = async () => {
    ...
    if (formType.value === 'create') {
        form.post(route('risk-hazard-class.store'), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
        ...
        form.put(route('risk-hazard-class.update', {id: form.id}), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
                showModal.value = false;
...
const handleDelete = (id) => {
    router.delete(route("risk-hazard-class.destroy", id), {
        preserveState: true,
    });
}
</script>
...
<template>
    <app-layout :title="tm('setting.risk.title')" :sub-title="tm('menu.hazardClasses')">
        <Table
            :data="tableData"
            :headers="headers"
            @edit="getRowInfo"
        ...
        >
            <template #actionArea>
                <help-button title="Tehlike Sınıfları — Nasıl Çalışır?" subtitle="Bir riskin fiziksel/kimyasal/biyolojik gibi hangi türden bir tehlikeden kaynaklandığını gösterir">
                    <p>Her risk, kaynaklandığı tehlikenin türünü gösteren bir <strong>tehlike sınıfı</strong>na atanabilir (Fiziksel, Kimyasal, Biyolojik, Ergonomik, Psikososyal, Mekanik, Elektriksel, Yangın/Patlama, Çevresel Etki, Bilgi Güvenliği İhlali, Operasyonel Aksama, Uygunluk/Yasal).</p>
                    <p>Sistemin geldiği <strong>korumalı</strong> (✔ işaretli) sınıflar silinemez; kendi eklediğiniz sınıflar kullanımda değilse serbestçe silinebilir.</p>
                </help-button>
