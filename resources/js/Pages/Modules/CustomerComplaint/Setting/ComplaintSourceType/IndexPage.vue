...
const handleSubmit = async () => {
    ...
    if (formType.value === 'create') {
        form.post(route('complaint-source-type.store'), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
        ...
        form.put(route('complaint-source-type.update', {id: form.id}), {
            onSuccess: () => {
                form.reset();
                v$.value.$reset();
                showModal.value = false;
...
const handleDelete = (id) => {
    router.delete(route("complaint-source-type.destroy", id), {
        preserveState: true,
    });
}
</script>
...
<template>
    <setting-layout :title="tm('title.complaintSourceTypePage.title')" :sub-title="tm('title.complaintSourceTypePage.subTitle')" :links="links">
        <template #actionArea>
            <help-button title="Şikayet Kaynakları — Nasıl Çalışır?" subtitle="Şikayetlerin kaynak sınıflandırması">
                <p>Her şikayet kaydı, kimden/nereden geldiğini gösteren bir <strong>kaynağa</strong> atanır (İç, Müşteri, Tedarikçi, Dağıtıcı).</p>
                <p>Şikayet formunda seçilen kaynağa göre ilgili taraf alanı (Müşteri/Tedarikçi/Dağıtıcı) zorunlu hale gelir; İç kaynaklı şikayetlerde hiçbiri istenmez.</p>
                <p>Sistemin geldiği <strong>korumalı</strong> (✔ işaretli) kaynaklar silinemez; kendi eklediğiniz kaynaklar kullanımda değilse serbestçe silinebilir.</p>
            </help-button>
        ...
        <teleport to="body">
            <Modal
                v-model="showModal"
                :header="tm('title.complaintSourceTypePage.title')"
                closeable
                close-button
            >
                <Form full-size>
