...
onBeforeMount(() => {
    if (props.modelValue) {
        select(props.options.find(i => i[props.optionKey] === props.modelValue))
    }
})
...
<template>
    <div class="relative space-y-1 gap-1 min-w-[12rem]" ref="target">
        ...
        <div @click="toggle"
            ...
            <div class="block overflow-hidden whitespace-nowrap">
                ...
                <span v-if="!modelValue"
                    v-text="selectText.length > 0 ? selectText : $t('form.selectInput.selectOption')"></span>
                <!--Selected-->
                <div v-else>
            ...
            <div class="flex items-center space-x-2">
                <!--Clear Button-->
                <font-awesome-icon v-if="selected" @click.stop="clear" icon="circle-xmark"
        ...
        <div v-if="showOptions"
            class="absolute z-50 bg-white dark:bg-slate-600 border shadow-lg rounded-lg py-1 w-full">
            <template v-for="option in options" :key="option">
                <div @click="select(option)"
                    class="flex justify-between items-center border-b last:border-b-0 last:rounded-b first:rounded-t py-1 px-2 hover:bg-slate-100 dark:hover:bg-slate-800/50 cursor-pointer">
                    <!--Label-->
                    <div>
